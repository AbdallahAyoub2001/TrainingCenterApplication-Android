...
public class InstructorSignUpActivity extends AppCompatActivity {
    ...
    private EditText etEmail, etFirstName, etLastName, etPassword, etConfirmPassword, mobileNumber, address, specialization, degree, courseToTeach;
    Button btnSignUp;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        courseToTeach = findViewById(R.id.etCourses);
        ...
        btnSignUp.setOnClickListener(v -> {
            ...
            String courses = courseToTeach.getText().toString();

            if (validate(email, firstName, lastName, password, confirmPassword, userPhoto, mobile_number, userAddress, courses)) {
                InstructorUser instructor = new InstructorUser(email, firstName, lastName, password, userPhoto, mobile_number, userAddress, Specialization, Degree, courses);

                try {
                    DataBaseHelper dataBaseHelper =new DataBaseHelper(InstructorSignUpActivity.this,"DB_TRAINING_CENTER",null,1);
                    dataBaseHelper.insertInstructor(instructor);
                } catch (IOException e) {
                    throw new RuntimeException(e);
                }

                startActivity(new Intent(InstructorSignUpActivity.this, InstructorHomeViewActivity.class));
                finish();
            }
    ...
    private boolean validate(String email, String firstName, String lastName, String password, String confirmPassword, ImageView photo, String mobile_number, String userAddress, String courses) {
        if (email.isEmpty()) {
        ...
        if(photo == null){
            Toast toast =Toast.makeText(InstructorSignUpActivity.this, "Please Attach your photo",Toast.LENGTH_SHORT);
            toast.show();
            return false;
        }
        if(mobile_number.isEmpty()){
            ...
            mobileNumber.setError("Phone number must be at least 10 digits long");
            mobileNumber.requestFocus();
            return false;
        }
        ...
        if(courses.isEmpty()){
            courseToTeach.setError("Courses is required");
            courseToTeach.requestFocus();
            return false;
        }
        return true;
